...
def get_legend(colormap, vars_, varname, lines=True, alphas=[1, 0.5], linestyles=['--', '-']):
    ...
    legend = []
    if lines:
        legend.append(mlines.Line2D([], [], color=color,
                                    alpha=alphas[0], linestyle=linestyles[0], label='Training'))
        legend.append(mlines.Line2D([], [], color=color, alpha=alphas[1],
                                    linestyle=linestyles[1], label='Validation'))
...
class AppendedHistory:
    ...
    def add_hist(self, var, history):
        ...
        for key in self.__history.keys():
            if key != self.__varname:
                self.__history[key][ind] = self.__history[key][ind] + \
                    history.history[key]

    def str_var(self):
        self.__history[self.__varname] = [
            str(var) for var in self.__history[self.__varname]]

    def order(self, reverse=False):
        sorted_inds, sorted_vars = zip(*sorted([(i, e) for i, e in enumerate(
            self.__history[self.__varname])], key=itemgetter(1), reverse=reverse))
        for key in self.__history.keys():
    ...
    def plot(self, title, mode, marker=False, range_=None, complementary=True, save=False, fname=""):
        ...
        with plt.style.context('ggplot'):
            ...
            def get_ylabel(mode): return 'Accuracy' if mode in [
                'acc', 'val_acc'] else 'Loss'
            #if get_ylabel(mode) == 'Loss':
            #    ax.set_yscale('log')
            ax.set_title(title)
            ...
            colormap = plt.cm.gist_rainbow(np.linspace(
                0, 1, len(self.__history[self.__varname])))
            ax.legend(title=self.__varname, handles=get_legend(
                colormap, self.__history[self.__varname], self.__varname, alphas=alphas, linestyles=linestyles))

            if mode.startswith('val_'):
                ind = 1
            else:
                ind = 0
            ...
            for val in self.__history[mode]:
                ...
                if range_ != None:
                    ...
                    x = np.arange(len(val))[slice_]

                color = next(color_iter)
                if range_ == None:
                    ax.plot(
                        val, marker=marker, linestyle=linestyles[ind], color=color, alpha=alphas[ind])
                else:
                    ax.plot(x, val[slice_], marker=marker,
                            linestyle=linestyles[ind], color=color, alpha=alphas[ind])
            if complementary:
                if ind == 1:
                    mode_2 = mode[4:]
                else:
                ...
                for val in self.__history[mode_2]:
                    ...
                    if range_ == None:
                        ax.plot(
                            val, marker=marker, linestyle=linestyles[1 - ind], color=color, alpha=alphas[1 - ind])
                    else:
                        ax.plot(x, val[slice_], marker=marker, linestyle=linestyles[1 -
                                                                                    ind], color=color, alpha=alphas[1 - ind])

        if save:
            fig.savefig(fname, dpi=300)
        plt.show()

    def plot_acc_loss(self, title, prefix_1, prefix_2=None, slice_=slice(None), save=False, fname="", log=False):
        ...
        with plt.style.context('ggplot'):
            ...
            colormap = plt.cm.gist_rainbow(np.linspace(
                0, 1, len(self.__history[self.__varname])))[slice_]
            ...
            for i, mode in enumerate(['acc', 'loss']):
                ...
                for val in self.__history[prefix_1 + mode][slice_]:
                    color = next(color_iter)
                    ax[i].plot(val, linestyle=linestyles[ind],
                               color=color, alpha=alphas[ind])
                del linestyles[ind]
                del alphas[ind]
                if prefix_2:
                    color_iter = iter(colormap)
            ...
            if prefix_2:
                legend = get_legend(
                    colormap, self.__history[self.__varname][slice_], self.__varname)
            else:
                legend = get_legend(
                    colormap, self.__history[self.__varname][slice_], self.__varname, lines=False)
            # , loc='lower right' bbox_to_anchor=(1.135, 1.0))
            ax[0].legend(title=self.__varname, handles=legend)
